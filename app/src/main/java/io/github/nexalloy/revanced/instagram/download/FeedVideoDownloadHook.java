...
public class FeedVideoDownloadHook {
    ...
    public static void installVideoUrlCaptureHook(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (DexKitCache.isCacheValid()) {
            ...
            if (cached != null && !cached.isEmpty()) {
                ...
                ModuleLog.line("(NA|DL|DexKit) VideoUrlCapture: " + cached.size() + " method(s) from cache");
        ...
        try {
            ...
            ModuleLog.line("(NA|DL|DexKit) VideoVersionIntf implementors found: " + classes.size());
                ...
                try {
                    ...
                    for (MethodData methodData : methods) {
                        try {
                            ...
                            ModuleLog.line("(NA|DL|DexKit) ✅ Hooked getUrl() on "
                                    + classData.getName());
                            hooked.add(m);
                        } catch (Throwable e) {
                            ModuleLog.line("(NA|DL|DexKit) ❌ Hook failed for "
                                    + classData.getName() + ": " + e.getMessage());
                        }
                    }
                } catch (Throwable e) {
                    ModuleLog.line("(NA|DL|DexKit) ❌ findMethod failed for "
                            + classData.getName() + ": " + e.getMessage());
                }
            }
            if (!hooked.isEmpty()) DexKitCache.saveMethods("VideoUrlCapture", hooked);
        } catch (Throwable e) {
            ModuleLog.line("(NA|DL|DexKit) ❌ installVideoUrlCaptureHook: " + e.getMessage());
        }
    ...
    private static void discoverDynamicMediaModel(DexKitBridge bridge, ClassLoader classLoader) {
        try {
            ...
            carouselCandidates.clear();
            carouselCandidates.addAll(mediaModel.listCandidates);
            ModuleLog.line("(NA|DL|DexKit) dynamic media dict="
                    + (liveTreeMediaDictClass == null ? "not found" : liveTreeMediaDictClass.getName()));
        } catch (Throwable t) {
            ModuleLog.line("(NA|DL|DexKit) dynamic media model resolution failed: " + t);
        }
    }
    ...
    private static void resolveVideoVersionsGetters(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        try {
            ...
            ModuleLog.line("(NA|DL|DexKit) video_versions getters="
                    + resolvedVideoVersionsGetters.size());
        } catch (Throwable t) {
            ModuleLog.line("(NA|DL|DexKit) video_versions getter resolution failed: " + t);
        }
    }
    ...
    private static void resolveIsVideoMethod(DexKitBridge bridge, ClassLoader classLoader) {
        try {
            ...
            ModuleLog.line("(NA|DL|DexKit) isVideo="
                    + (resolvedIsVideoMethod == null ? "not found" : resolvedIsVideoMethod.getName()));
        } catch (Throwable t) {
            ModuleLog.line("(NA|DL|DexKit) isVideo resolution failed: " + t);
        }
    }
    ...
    private static void resolveUsernameGetter(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        try {
            ...
            if (userMethods.isEmpty()) {
                ModuleLog.line("(NA|DL|Username) ❌ username_missing_during_update not found");
                return;
            }
            ...
            ModuleLog.line("(NA|DL|Username) userClass=" + userClass.getName());
            ...
            try {
                ...
                if (!ugMethods.isEmpty()) {
                    ...
                    ModuleLog.line("(NA|DL|Username) userUsernameGetter=" + UserUtils.userUsernameGetter.getName()
                            + " (excluded " + readsFullName.size() + " full_name getter(s))");
                } else {
                    ModuleLog.line("(NA|DL|Username) ❌ userUsernameGetter not found via -265713450");
                }
            } catch (Throwable t) {
                ModuleLog.line("(NA|DL|Username) ❌ userUsernameGetter resolution: " + t);
            }
            ...
            ModuleLog.line("(NA|DL|Username) ❌ resolveUsernameGetter: " + t);
        }
    }
    ...
    private static void resolveMediaAuthorGetter(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        try {
            ...
            if (!res.isEmpty()) {
                ...
                ModuleLog.line("(NA|DL|Username) ✅ mediaAuthorGetter (Media.\"user\"): " + m.getName());
            } else {
                ModuleLog.line("(NA|DL|Username) mediaAuthorGetter: no Media \"user\" getter (older build)");
            }
        } catch (Throwable t) {
            ModuleLog.line("(NA|DL|Username) ❌ mediaAuthorGetter: " + t);
        }
    }
        ...
        while (!queue.isEmpty()) {
            ...
            for (Method m : curr.getDeclaredMethods()) {
                ...
                if (m.getParameterCount() == 0 && m.getReturnType().equals(userClass)) {
                    ...
                    ModuleLog.line("(NA|DL|Username) ✅ Resolved dictUserGetter: " + m.getName());
                    return;
                }
            }
        ...
        if (liveTreeMediaDictClass != null) {
            try {
                ...
                if (!results.isEmpty()) {
                    ...
                    dictUserGetter = m;
                    DexKitCache.saveMethod("DictUserGetter", m);
                    ModuleLog.line("(NA|DL|Username) ✅ Resolved dictUserGetter (concrete class): " + m.getName());
                    return;
                }
            } catch (Throwable t) {
                ModuleLog.line("(NA|DL|Username) ❌ dictUserGetter DexKit lookup: " + t);
            }
        }

        ModuleLog.line("(NA|DL|Username) ❌ Failed to resolve dictUserGetter in hierarchy");
    }
