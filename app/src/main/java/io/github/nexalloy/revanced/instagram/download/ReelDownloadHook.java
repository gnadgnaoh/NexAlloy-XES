    ...
    public void install(DexKitBridge bridge, ClassLoader classLoader) {
        ...
        if (DexKitCache.isCacheValid()) {
            ...
            if (cached != null) {
                ...
                ModuleLog.line("(NA|Reel) ✅ hooked: " + hookMethod.getDeclaringClass().getName() + "." + hookMethod.getName());
                return;
            }
        }
        ...
        try {
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(NA|Reel) ❌ ClipsOrganicMediaItemViewMoreOptionsController not found");
                return;
            }
            ...
            ModuleLog.line("(NA|Reel) string matches=" + methods.size()
            ...
            for (Class<?> cand : candidates) {
                ...
                if (voidMedia > 0) {
                    ModuleLog.line("(NA|Reel) candidate " + cand.getName()
            ...
            if (target == null) {
                ModuleLog.line("(NA|Reel) ❌ hook method (Media, ButtonAdder)V not found");
                return;
            }
            ...
            ModuleLog.line("(NA|Reel) ❌ install: " + t);
        }
    }
    ...
    private static void installReduceOptionsListPatch(DexKitBridge bridge, ClassLoader classLoader) {
        try {
            ...
            if (downloadOption == null && copyLinkOption == null) {
                ModuleLog.line("(NA|Reel) ❌ DOWNLOAD/COPY_LINK enum values not found");
                return;
            }
            ...
            XC_MethodHook hook = new XC_MethodHook() {
                ...
                protected void afterHookedMethod(MethodHookParam param) {
                    ...
                    try {
                        ...
                        ModuleLog.line("(NA|Reel) ❌ options-list patch failed: " + t);
                    }
                }
            };
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(NA|Reel) ⚠️ Reduced options-list builder not found");
                return;
            }
            ...
            FeatureStatusTracker.setHooked("ReelDownload");
            ModuleLog.line("(NA|Reel) ✅ Options-list patch hooked: " +
                    target.getDeclaringClass().getName() + "." + target.getName());

        } catch (Throwable t) {
            ModuleLog.line("(NA|Reel) ❌ installReduceOptionsListPatch: " + t);
        }
    }
    ...
    private static void installGateHook(DexKitBridge bridge, ClassLoader classLoader,
        ...
        XC_MethodHook hook = new XC_MethodHook() {
            ...
            protected void beforeHookedMethod(MethodHookParam param) {
                ModuleLog.line("(NA|Reel|DEBUG) gate fired: " + cacheKey + " enabled=" + FeatureFlags.enableReelDownload);
        ...
        try {
            ...
            if (methods.isEmpty()) {
                ModuleLog.line("(NA|Reel) ⚠️ Gate method not found for config " + configId);
                return;
            }
            ...
            FeatureStatusTracker.setHooked("ReelDownload");
            ModuleLog.line("(NA|Reel) ✅ Gate unlocked: " +
                    target.getDeclaringClass().getName() + "." + target.getName() + " -> " + forcedResult);

        } catch (Throwable t) {
            ModuleLog.line("(NA|Reel) ❌ installGateHook(" + cacheKey + "): " + t);
        }
    }
