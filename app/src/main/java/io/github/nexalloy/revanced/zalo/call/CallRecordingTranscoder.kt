...
internal object CallRecordingTranscoder {
    ...
    /**
     * Rebuilds the RIFF and `data` chunk length fields from the file's real
     * size and re-validates.
     *
     * Some native ZRTC writers stream PCM samples into the WAV but never
     * back-patch the placeholder length fields when the recording stops, so the
     * finished file fails [isPcmWave] even though it holds valid audio.
     * When the file already validates, or has no recoverable `data` chunk,
